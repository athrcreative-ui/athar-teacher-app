...
function resourceTone(resource: Resource, action?: string): string {
  const value = `${action ?? ''} ${resource.format ?? ''}`.toLowerCase();
  if (value.includes('نشاط') || value.includes('لعبة')) return 'activity';
  if (value.includes('تقويم') || value.includes('اختبر')) return 'quiz';
  if (value.includes('عرض') || value.includes('ppt') || value.includes('powerpoint') || value.includes('بور')) return 'presentation';
  if (value.includes('بوكليت') || value.includes('booklet')) return 'booklet';
  return 'default';
}
...
function SafeResourceLink({ resource, action = 'افتح المورد' }: { resource: Resource; action?: string }) {
  ...
  const tone = resourceTone(resource, action);
  ...
  return (
    <article className={`resource-card resource-card--${tone}`}>
      <div className="resource-card__body">
        ...
        <span className="resource-icon resource-card__icon"><Icon name="file" /></span>
      ...
      <SafeExternalLink
        className="text-link resource-action resource-action--button"
...
export function ResourceList({ resources, action }: { resources: Resource[]; action?: string }) {
  ...
  return (
    <div className="resource-list resource-list--resources">
      {safeResources.map((resource) => <SafeResourceLink action={action} key={resource.id} resource={resource} />)}
    </div>
  );
}

function googleDriveImageUrl(value: string): string | null {
  try {
    const url = new URL(value);
    if (url.hostname !== 'drive.google.com') return null;
    const parts = url.pathname.split('/').filter(Boolean);
    if (parts[0] !== 'file' || parts[1] !== 'd' || !parts[2]) return null;
    return `https://drive.google.com/thumbnail?id=${encodeURIComponent(parts[2])}&sz=w2000`;
  } catch {
    return null;
  }
}
...
function SummaryImage({ resource }: { resource: SummaryResource }) {
  ...
  if (!isSafeContentUrl(resource.url)) return null;
  const imageUrl = googleDriveImageUrl(resource.url) ?? resource.url;
...
export function SummaryResources({ resources }: { resources: SummaryResource[] }) {
  return (
    <div className="resource-list resource-list--summary">
...
function YouTubeCard({ resource, videoId }: { resource: VideoResource; videoId: string }) {
  ...
  return (
    <article className="video-card video-card--youtube">
...
function DriveMediaCard({ resource, previewUrl }: { resource: VideoResource; previewUrl: string }) {
  ...
  return (
    <article className={`video-card video-card--drive${isAudio ? ' video-card--audio' : ''}`}>
      <div className="video-card__heading">
        <span className="resource-icon"><Icon name="video" /></span>
        <div>
          <h3>{resource.title}</h3>
...
export function VideoResources({ resources }: { resources: VideoResource[] }) {
  return (
    <div className="resource-list resource-list--video">
      {resources.map((resource) => {
        ...
        if (resource.kind === 'direct') {
          return (
            <article className="video-card video-card--direct" key={resource.id}>
              <h3>{resource.title}</h3>
              {resource.description && <p>{resource.description}</p>}
...
export function HomeworkList({ items }: { items: HomeworkItem[] }) {
  return (
    <div className="resource-list resource-list--homework">
      {items.map((item) => (
